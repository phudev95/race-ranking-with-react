...
const UserInformationWithoutMemo = ({
  raceRankingUser
}) => {
  return (
    <div className="user-information" key={raceRankingUser.id}>
      ...
      <meter value={raceRankingUser.progress % LAP_LENGTH} min="0" max={LAP_LENGTH} />
      <i className="km">{raceRankingUser.progress.toFixed(2)} km (Round {Math.ceil(raceRankingUser.progress / LAP_LENGTH)})</i>
    </div>
  );
};
